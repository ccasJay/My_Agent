...
- HTTP 和 Shell 仍是同步阻塞调用，取消可能要等当前调用返回。
...
`session/new`、`session/load` 和 `session/resume` 都要求：
...
一条连接可以同时加载多个 Session，但整个 `agent-acp` 进程最多运行一个
Prompt；并发提交返回 Server busy 错误。
...
`session/cancel` 时，Prompt 进入取消流程。已取消请求的迟到响应会被忽略并
写入 stderr 诊断，不会影响后续 Prompt。
...
Prompt、权限响应、并发拒绝、取消、重启后的 list/load 和输入校验。若 Client
报告 JSON 解析错误，优先确认它只读取 stdout，且没有把 stderr 合并进去。
