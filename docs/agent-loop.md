...
停止通过 StopToken 传播。当前 OpenAI Provider 的 HTTP POST 使用 libcurl
进度回调中止传输；默认 Shell Executor 终止独立子进程组。接口仍兼容只实现
旧 `query()` 的第三方 Provider，这类实现可能要等调用返回后才观察到停止。
无论底层何时返回，`Stopped` 都优先于继续请求模型或提交完成状态。
...
`run_shell()` 通过 `fork()` 创建子进程，以管道合并 stdout/stderr，并由
`waitpid()` 获取退出状态。带工作目录的重载只在子进程中 `chdir()`，不会
修改宿主进程。可取消重载把 stdin 指向 `/dev/null`，关闭非标准文件描述符，
并让子进程建立独立进程组；取消时先发送 SIGTERM，1 秒后升级为 SIGKILL。
该函数不因普通命令失败而抛异常，而是返回 `ProcessResult`：

- 正常退出：`termination=Exited`，保存 exit code；
- 信号终止：`termination=Signaled`，保存 signal number；
- pipe/fork/read/wait 失败：`termination=ExecutionError` 并保存错误文本；
