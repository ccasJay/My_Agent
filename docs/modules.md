...
- `HttpClient::post()`：初始化 libcurl、配置 headers/body、执行请求。
...
HTTP 请求同步访问网络。curl 失败、非 2xx、JSON 解析失败或响应字段不符合
预期时抛异常。请求头包含 Bearer token，日志不应打印该值。
...
- `run_shell()`：以子进程执行，可指定 cwd，并限制保存输出。
...
策略本身不执行命令。`run_shell()` 会启动本地子进程，合并 stdout/stderr；
...
保持协议纯净。close/EOF 会协作取消并 join；同步 HTTP/Shell 返回前不能
强制中断。首版能力边界详见 [ACP 接入指南](acp-integration.md)。
...
- `SessionCommand` / `SessionCommandKind`：`/new`、`/sessions`、`/resume`。

### 关键函数

- `tui::run(SessionManager&, model_name)`：TUI composition root 和事件循环。
- `parse_session_command()`：解析会话命令及参数错误。
...
从 `TuiSession` 取快照，再调用 `tui_view.cpp` 的渲染函数组合界面。

### 异常与副作用

该模块占用终端全屏并启动动画线程。Session 命令异常被转换为界面 notice；
...
- [`include/tui/session_command.hpp`](../include/tui/session_command.hpp)
- [`src/session_command.cpp`](../src/session_command.cpp)

### 测试索引

- [`tests/test_session_command.cpp`](../tests/test_session_command.cpp)
