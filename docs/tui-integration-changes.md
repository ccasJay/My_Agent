...
- 支持 StopToken 贯穿模型和 Shell 调用；HTTP 可中止传输，Shell 可终止进程组。
...
- OpenAI Provider 通过 libcurl 进度回调中止正在进行的 HTTP 请求。
- 默认 Shell Executor 先终止独立进程组，1 秒后仍未退出则升级为 SIGKILL。
- 旧 Provider 兼容接口可能要等当前调用返回；停止状态仍优先于继续请求模型
  或提交完成状态。
...
继续排空非常重要：如果父进程停止读取，高输出子进程可能阻塞在 `write()`，随后 `waitpid()` 也会一直等待。
...
- HTTP 与 Shell API 仍为同步调用；旧 Provider 可能不能中途取消。
- Shell 支持取消，但没有独立的运行超时配置。
- Shell 输出不是实时流式读取。
- TUI 输入框为单行，不支持多行编辑。
- 渲染后的日志块只保存在当前进程内；对话消息由 Session Store 持久化。
