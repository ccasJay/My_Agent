...
1. 顶栏：应用名称和当前状态。
2. 日志区：以语义块展示任务、模型回复、命令、执行结果和最终结论。
3. 输入区：空闲时输入任务；运行时展示 Grok 风格的 turn status。
4. 状态栏：展示模型、step 和日志跟随状态。
5. 快捷键栏：根据空闲或运行状态显示可用操作。
...
| 空闲 | `Tab` / `Shift+Tab` | 在 Prompt 和 Scrollback 之间切换焦点 |
