...
空闲且焦点在 Prompt 时，若光标所在 token 以 `/` 开头，会在输入框上方显示
已注册命令列表，并按命令名**前缀**（大小写敏感）过滤。

| 按键 | 弹层打开时 |
| --- | --- |
| `↑` / `↓` | 移动高亮项（不环形） |
| `Tab` / `Enter` | 将选中项**补全**到输入框并关闭弹层（不立刻执行） |
| `Esc` | 仅关闭弹层，保留已输入文本 |

弹层关闭后，`Enter` 才按原逻辑执行斜杠命令或提交普通任务；`↑`/`↓` 恢复
为 Prompt 历史，`Tab` 恢复为 Prompt/Scrollback 切换。有参命令（如
`/resume`）补全后会带尾随空格，便于继续输入参数。
...
实现使用六色浅色调色板：纸面 `#F8FAFC`、墨色 `#1F2937`、蓝图
`#2854C5`、完成 `#087F72`、审核 `#965500`、故障 `#B4233A`。其中蓝图用于
Task/System 等结构信号，墨色用于 Assistant/Observation，审核用于 Command，
完成用于 Final，故障用于 Error；状态和操作也沿用相同语义。
